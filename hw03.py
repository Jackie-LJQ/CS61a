...
def pingpong(n):
    ...
    def helper(result, i, step):
        if i==n:
            ...
            return helper(result + step, i + 1, next_step(i+1, step))
    def next_step(i, step):
...
def pingpong3(n):
    if n <= 7:
        ...
        return direction(n-1)+pingpong3(n-1)
...
def pingpong2(n):
    def helper(x, i, step):
        if i == n:
            return x
        elif (i+1) % 7== 0 or num_sevens(i+1) > 0:
            return helper(x+step, i + 1, -step)
        else:
            return helper(x+step, i + 1, step)
    return helper(1,1,1)
...
def count_change(amount):
    ...
    def count(amount, i):
        if amount == 0:
            ...
            return count(amount - 2**i, i) + count(amount, i - 1)
